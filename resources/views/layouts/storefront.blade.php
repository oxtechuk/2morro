...
<html lang="ar" dir="rtl">
    <head>
        ...
        @php
            $siteMetaTitle = \App\Models\Setting::get('meta_title', 'متجر ومركز تمورو | تنمية مهارات الطفل وتخاطب وتأهيل');
            $siteMetaDescription = \App\Models\Setting::get('meta_description', 'مركز ومتجر تمورو لتنمية مهارات الطفل، جلسات تخاطب وتعديل سلوك وتدخل مبكر وأدوات وشيتات تعليمية بإشراف أ. هبة الله أكرم');
        ...
        <style>
            ...
            body, html, input, button, select, textarea, span, p, h1, h2, h3, h4, h5, h6, a {
                font-family: 'Bahij TheSansArabic', 'Cairo', system-ui, -apple-system, sans-serif !important;
            }
            .bg-\[\#1360e2\],
            .bg-\[\#2563eb\],
            .bg-\[\#2563ea\],
    ...
    <body class="antialiased bg-[#F8FAFC] text-slate-800 font-sans selection:bg-blue-600 selection:text-white min-h-screen flex flex-col justify-between" 
        ...
        @php
            ...
            $topBarText = \App\Models\Setting::get('top_bar_text', 'فروع مركز 2morro بالإسكندرية (الإبراهيمية - البيطاش - سيدي بشر) • جلسات وتقييمات في المركز وأونلاين • شحن مجاني للطلبات فوق 550 ج.م');
            $topBarEnabled = \App\Models\Setting::get('top_bar_enabled', '1');
        @endphp

        @if($topBarEnabled !== '0' && !empty($topBarText))
        ...
        <div class="bg-[#2563ea] text-white text-[11px] font-bold py-1.5 px-2 overflow-hidden shadow-2xs select-none" style="background-color: #2563ea !important; color: #ffffff !important;" dir="ltr">
            <div class="flex overflow-hidden whitespace-nowrap w-full">
                <div class="inline-flex items-center animate-top-marquee gap-8" dir="rtl">
                    <span class="whitespace-nowrap">{{ $topBarText }}</span>
        ...
        <header class="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-2xs transition-all">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center justify-between h-15 sm:h-16 gap-4">
                    ...
                    <nav class="hidden lg:flex items-center justify-center gap-5 xl:gap-7 text-sm font-bold text-slate-700">
                        
                        <a href="{{ route('home') }}" class="py-2 px-0.5 transition-colors whitespace-nowrap {{ request()->routeIs('home') ? 'text-[#2563ea] border-b-2 border-[#2563ea] font-black' : 'text-slate-700 hover:text-[#2563ea]' }}">
                            الرئيسية
                        </a>

                        <!-- Categories Dropdown (Alpine.js) -->
                        <div class="relative" x-data="{ catMenuOpen: false }" @click.away="catMenuOpen = false">
                            <button @click="catMenuOpen = !catMenuOpen" class="py-2 px-0.5 hover:text-[#2563ea] transition-colors flex items-center gap-1.5 text-slate-700 font-bold whitespace-nowrap">
                                <span>الأقسام والتصنيفات</span>
                                <svg class="w-3.5 h-3.5 text-slate-400 transition-transform duration-200" :class="catMenuOpen ? 'rotate-180 text-[#2563ea]' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </button>
                        ...
                        <a href="{{ route('search', ['category' => 'educational-tools']) }}" class="py-2 px-0.5 transition-colors whitespace-nowrap {{ request()->get('category') === 'educational-tools' ? 'text-[#2563ea] border-b-2 border-[#2563ea] font-black' : 'text-slate-700 hover:text-[#2563ea]' }}">
                            الأدوات التعليمية
                        </a>

                        <a href="{{ route('search', ['category' => 'digital-worksheets']) }}" class="py-2 px-0.5 transition-colors whitespace-nowrap {{ request()->get('category') === 'digital-worksheets' ? 'text-[#2563ea] border-b-2 border-[#2563ea] font-black' : 'text-slate-700 hover:text-[#2563ea]' }}">
                            الشيتات الرقمية
                        </a>

                        <a href="{{ route('search', ['category' => 'educational-bundles']) }}" class="py-2 px-0.5 transition-colors whitespace-nowrap {{ request()->get('category') === 'educational-bundles' ? 'text-[#2563ea] border-b-2 border-[#2563ea] font-black' : 'text-slate-700 hover:text-[#2563ea]' }}">
                            باقات التوفير
                        </a>

                        <a href="{{ route('booking.index') }}" class="ms-1 px-4 py-2 rounded-xl text-white text-xs font-black transition-all shadow-xs hover:opacity-90 hover:scale-[1.02] active:scale-95 whitespace-nowrap" style="background-color: #f76d11;">
                            حجز استشارة
                        </a>
                    ...
                    <div class="flex items-center gap-1 sm:gap-2">
                        ...
                        <!-- Search Icon -->
                        <button @click="searchOpen = !searchOpen" title="بحث في المتجر" class="p-2 text-slate-700 hover:text-[#2563ea] transition-colors rounded-xl flex items-center justify-center">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                        </button>

                        <!-- User Account / My Account Dropdown -->
                        <div class="relative" x-data="{ userMenuOpen: false }" @click.away="userMenuOpen = false">
                            @auth
                                <button @click="userMenuOpen = !userMenuOpen" title="حسابي ({{ Auth::user()->name }})" class="p-2 text-slate-700 hover:text-[#2563ea] transition-colors rounded-xl flex items-center justify-center">
                            ...
                            @else
                                <a href="{{ route('login') }}" title="تسجيل الدخول / حسابي" class="p-2 text-slate-700 hover:text-[#2563ea] transition-colors rounded-xl flex items-center justify-center">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                    </svg>
                                </a>
                        ...
                        <!-- Clean Stroke Cart Bag Button & Badge -->
                        <button @click="cartOpen = true" title="سلة المشتريات" class="relative p-2 text-slate-700 hover:text-[#2563ea] transition-colors rounded-xl flex items-center justify-center group">
                            <svg class="w-6 h-6 transform group-hover:scale-105 transition-transform" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                            </svg>

                            <!-- Accurate Pin-point Badge on top corner -->
                            <span class="absolute top-0.5 -left-0.5 min-w-[18px] h-[18px] px-1 bg-[#EF4444] text-white text-[10px] font-black rounded-full flex items-center justify-center shadow-xs border-2 border-white">
                                {{ $cartCount }}
                            </span>
                        </button>
        ...
        <!-- ========================================================================= -->
        <!-- ==================== COMPACT UNIFIED LUXURY FOOTER ==================== -->
        <footer class="bg-slate-50 text-slate-800 border-t border-slate-200/90 mt-10 sm:mt-14 pt-8 pb-8 px-4 sm:px-6 lg:px-8 relative" style="background-color: #f8fafc; color: #1e293b;">
            <div class="max-w-7xl mx-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-8 py-6 sm:py-8 text-right items-start">
                    ...
                    <div class="md:col-span-4 flex flex-col gap-3">
                        <div class="inline-block bg-white p-2 rounded-xl self-start shadow-xs border border-slate-200">
                            <img src="{{ asset($storeLogo) }}?v={{ $logoVersion }}" alt="2morro" style="max-height: 44px; width: auto; object-fit: contain;" class="h-9 sm:h-11 w-auto object-contain">
                        </div>
                        
                        <p class="text-xs text-slate-600 font-semibold leading-relaxed">
                            <strong class="text-slate-900 font-extrabold">مركز ومتجر 2morro:</strong> تأهيل وتنمية مهارات الأطفال وتوفير أفضل الوسائل والألعاب التعليمية والشيتات بإشراف أ. هبة الله أكرم.
                        </p>

                        <!-- Linktree Official Button -->
                        <div>
                            <a href="https://linktr.ee/hebaalla?subscribe" target="_blank" rel="noopener noreferrer" 
                               class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 border border-slate-200 text-xs font-bold transition-all group">
                                <svg class="w-3.5 h-3.5 text-[#2563ea] group-hover:rotate-12 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg>
                                <span>روابط المركز الرسمية (Linktree)</span>
                            </a>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-1.5 pt-0.5">
                            ...
                            <a href="https://www.instagram.com/hebaallaakrm/" target="_blank" rel="noopener noreferrer" class="w-8 h-8 rounded-lg bg-white text-slate-600 hover:text-pink-600 hover:bg-pink-50 border border-slate-200 flex items-center justify-center transition-all shadow-2xs" title="انستجرام">
                            ...
                            <a href="https://youtube.com/c/2Morro?sub_confirmation=1" target="_blank" rel="noopener noreferrer" class="w-8 h-8 rounded-lg bg-white text-slate-600 hover:text-red-600 hover:bg-red-50 border border-slate-200 flex items-center justify-center transition-all shadow-2xs" title="يوتيوب">
                            ...
                            <a href="tel:01550504512" class="w-8 h-8 rounded-lg bg-white text-slate-600 hover:text-[#2563ea] hover:bg-blue-50 border border-slate-200 flex items-center justify-center transition-all shadow-2xs" title="اتصال هاتفي">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                            </a>
                        </div>
                    </div>

                    <!-- Col 2: Compact Branches (فروع المركز - قائمة مدمجة ومرتبة) (4 cols) -->
                    <div class="md:col-span-4 flex flex-col gap-2.5">
                        <h4 class="text-xs font-black text-slate-900 flex items-center gap-1.5">
                            <span class="w-1.5 h-3.5 bg-[#2563ea] rounded-full"></span>
                            فروع المركز بالإسكندرية
                        </h4>

                        <div class="space-y-1.5 text-xs">
                            <!-- Branch 1 -->
                            <div class="bg-white border border-slate-200/90 p-2.5 rounded-xl shadow-2xs">
                                <div class="flex items-center justify-between gap-1">
                                    <span class="font-extrabold text-slate-900 text-[11px]">  الإبراهيمية (الرئيسي):</span>
                                    <a href="tel:01550504512" class="text-[11px] font-mono font-bold text-[#2563ea] hover:underline" dir="ltr">01550504512</a>
                                </div>
                                <span class="text-slate-500 text-[10px] block mt-0.5">أول لاجتيه من شارع أبو قير (فوق سنترال إياد)</span>
                            </div>

                            <!-- Branch 2 -->
                            <div class="bg-white border border-slate-200/90 p-2.5 rounded-xl shadow-2xs">
                                <div class="flex items-center justify-between gap-1">
                                    <span class="font-extrabold text-slate-900 text-[11px]">  أول البيطاش (غرب):</span>
                                    <a href="tel:01064580472" class="text-[11px] font-mono font-bold text-[#2563ea] hover:underline" dir="ltr">01064580472</a>
                                </div>
                                <span class="text-slate-500 text-[10px] block mt-0.5">أمام بنك القاهرة - عمارة مركز القلب</span>
                            </div>

                            <!-- Branch 3 -->
                            <div class="bg-white border border-slate-200/90 p-2.5 rounded-xl shadow-2xs">
                                <div class="flex items-center justify-between gap-1">
                                    <span class="font-extrabold text-slate-900 text-[11px]">  سيدي بشر (شرق):</span>
                                    <a href="tel:01508074512" class="text-[11px] font-mono font-bold text-[#2563ea] hover:underline" dir="ltr">01508074512</a>
                                </div>
                                <span class="text-slate-500 text-[10px] block mt-0.5">أول نفق جمال عبد الناصر (فوق رؤية سكان)</span>
                            </div>
                        </div>
                    </div>

                    <!-- Col 3: Quick Links & Store Sections (2 cols) -->
                    <div class="md:col-span-2 flex flex-col gap-2.5">
                        <h4 class="text-xs font-black text-slate-900 flex items-center gap-1.5">
                            <span class="w-1.5 h-3.5 bg-slate-700 rounded-full"></span>
                            أقسام المتجر
                        </h4>
                        <ul class="text-xs space-y-2 text-slate-600 font-bold">
                            <li><a href="{{ route('search', ['category' => 'educational-tools']) }}" class="hover:text-[#2563ea] transition-colors flex items-center gap-1.5"><span class="w-1 h-1 rounded-full bg-slate-400"></span> الألعاب والأدوات</a></li>
                            <li><a href="{{ route('search', ['category' => 'digital-worksheets']) }}" class="hover:text-[#2563ea] transition-colors flex items-center gap-1.5"><span class="w-1 h-1 rounded-full bg-slate-400"></span> شيتات PDF</a></li>
                            <li><a href="{{ route('search', ['category' => 'educational-bundles']) }}" class="hover:text-[#2563ea] transition-colors flex items-center gap-1.5"><span class="w-1 h-1 rounded-full bg-slate-400"></span> باقات التوفير</a></li>
                            <li class="pt-1">
                                <a href="{{ route('booking.index') }}" class="inline-flex items-center gap-1 text-[11px] font-black text-[#2563ea] bg-blue-50 border border-blue-200 px-2.5 py-1.5 rounded-lg hover:bg-[#2563ea] hover:text-white transition-all">
                    ...
                    <!-- Col 4: Working Hours & WhatsApp Button (2 cols) -->
                    <div class="md:col-span-2 flex flex-col gap-2.5">
                        <h4 class="text-xs font-black text-slate-900 flex items-center gap-1.5">
                            <span class="w-1.5 h-3.5 bg-slate-700 rounded-full"></span>
                            مواعيد العمل
                        ...
                        <div class="bg-white border border-slate-200 p-2.5 rounded-xl shadow-2xs">
                            <span class="font-black text-slate-900 text-xs block">12:00 ظ - 9:00 م</span>
                            <span class="text-[10px] text-slate-500 font-bold block mt-0.5">يومياً (عدا الجمعة)</span>
                        </div>
                ...
                <!-- 3. Bottom Bar: Copyright & Developer Info (شريط سفلي مدمج) -->
                <div class="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500 font-bold text-center sm:text-right">
                    <p class="text-[11px] text-slate-600">
                        جميع الحقوق محفوظة &copy; {{ date('Y') }} <span class="text-slate-900 font-black">مركز ومتجر 2morro</span> | إشراف أ. هبة الله أكرم
                    </p>
                    
                    <p class="text-[11px] text-slate-500 flex items-center gap-1">
                        <span>تم التطوير من قبل</span>
                        <a href="https://oxtech.uk" target="_blank" rel="noopener noreferrer" class="font-black text-[#2563ea] hover:text-blue-700 transition-colors inline-flex items-center gap-1 hover:underline" title="Ox Tech">
                            <span>Ox Tech</span>
                            <svg class="w-3 h-3 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                        </a>
                    </p>
                </div>
